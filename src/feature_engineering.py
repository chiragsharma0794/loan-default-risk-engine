...
def engineer_features(df):
    ...
    if 'loan_amnt' in df.columns and 'annual_inc' in df.columns:
        df['loan_to_inc_ratio'] = df['loan_amnt'] / (df['annual_inc'] + 1).where(df['annual_inc'] > 0)
        
    # 3. Credit Utilization Ratios
    if 'total_bal_ex_mort' in df.columns and 'annual_inc' in df.columns:
        df['total_bal_to_inc_ratio'] = df['total_bal_ex_mort'] / (df['annual_inc'] + 1).where(df['annual_inc'] > 0)
    ...
    if 'emp_length' in df.columns:
        df['emp_length_num'] = pd.to_numeric(df['emp_length'].str.extract(r'(\d+)')[0], errors='coerce').astype(float)
        df.loc[df['emp_length'].eq('< 1 year').fillna(False), 'emp_length_num'] = 0.0
        
    return df

if __name__ == "__main__":
    raise SystemExit("Use the training-fitted preprocessing contract; no pooled CSV engineering.")
